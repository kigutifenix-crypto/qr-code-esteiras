...
export default function BikesPage() {
  ...
  const [bikes, setBikes] = useState<Treadmill[]>([])
  ...
  const filteredBikes = filterTreadmills(bikes, filters)

  // Seleção de máquinas
  const allVisibleIds = filteredBikes.map(b => b.id)
      ...
      if (archiveMode === 'archive') {
        ...
        try {
          await createLog({
            ...
            entity: 'treadmill',
            entityId: deleteId,
            details: `Archived bike ${deleteId}`,
          })
        } catch (e) {
...
<head>
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .company { font-size: 13px; font-weight: 800; color: #10b981; letter-spacing: 2px; text-transform: uppercase; }
    .badge { font-size: 12px; font-weight: 700; color: #064e3b; background: #d1fae5; padding: 3px 14px; border-radius: 20px; border: 1px solid #6ee7b7; }
...
<body>
  ...
  const printAllLabels = () => generateLabels(filteredBikes)

  const printSelectedLabels = () => {
    const selected = filteredBikes.filter(b => selectedIds.has(b.id))
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Gerencie todas as bikes cadastradas no sistema
          </p>
        </div>
        <div className="flex items-center gap-2 flex-wrap">
          ...
          >
            <Printer className="h-4 w-4" />
            {printingLabels ? 'Gerando...' : `Imprimir Todas (${filteredBikes.length})`}
          </Button>
          {canCreate && (
            <Button asChild>
      ...
      {/* Selection info bar */}
      {selectedIds.size > 0 && (
        <div className="flex items-center justify-between rounded-lg border border-emerald-200 bg-emerald-50 px-4 py-2 text-sm text-emerald-700 dark:border-emerald-800 dark:bg-emerald-950 dark:text-emerald-300">
        ...
        <CardContent className="p-0">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-12 text-center">
              <Bike className="h-12 w-12 text-muted-foreground/50 mb-4" />
              <h3 className="text-lg font-semibold">Nenhuma bike encontrada</h3>
              <p className="text-muted-foreground text-sm mt-1">
                {filters.search || filters.status !== 'all'
                  ? 'Tente ajustar os filtros de busca'
                  : 'Comece cadastrando uma nova bike'}
              </p>
            ...
            <Table>
                ...
                <TableRow className="hover:bg-transparent">
                  <TableHead className="w-[48px]">
                    <input
                      ...
                      className="h-4 w-4 cursor-pointer rounded border-2 border-slate-400 bg-white accent-emerald-600 dark:bg-slate-700 dark:border-slate-500"
                    />
                  </TableHead>
              ...
              <TableBody>
                {filteredBikes.map((bike) => (
                  <TableRow
                    key={bike.id}
                    className={`group cursor-pointer ${selectedIds.has(bike.id) ? 'bg-emerald-50/50 dark:bg-emerald-950/20' : ''}`}
                    tabIndex={0}
                    role="button"
                    onClick={(e) => {
                      const target = e.target as HTMLElement
                      if (target.closest('button, a, [role="checkbox"]')) return
                      router.push(`/dashboard/esteiras/${bike.id}?back=/dashboard/bikes`)
                    }}
                    onKeyDown={(e) => {
                      if (e.key === 'Enter') {
                        router.push(`/dashboard/esteiras/${bike.id}?back=/dashboard/bikes`)
                  ...
                  >
                    <TableCell onClick={(e) => e.stopPropagation()}>
                      <input
                        ...
                        onChange={() => toggleSelect(bike.id)}
                        aria-label={`Selecionar ${bike.name}`}
                        className="h-4 w-4 cursor-pointer rounded border-2 border-slate-400 bg-white accent-emerald-600 dark:bg-slate-700 dark:border-slate-500"
                      />
                    </TableCell>
                    <TableCell>
                    ...
                    <TableCell className="text-muted-foreground text-sm">
                      {format(bike.createdAt, "dd/MM/yyyy", { locale: ptBR })}
                    </TableCell>
                      ...
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          <DropdownMenuItem asChild>
                            <Link href={`/dashboard/esteiras/${bike.id}?back=/dashboard/bikes`}>
                              <Eye className="mr-2 h-4 w-4" />
                              Visualizar
                            </Link>
                          </DropdownMenuItem>
                          <DropdownMenuItem asChild>
                            <Link href={`/dashboard/esteiras/${bike.id}/qrcode?back=/dashboard/bikes`}>
                          ...
                          {canEdit && (
                            <DropdownMenuItem asChild>
                              <Link href={`/dashboard/esteiras/${bike.id}/editar?back=/dashboard/bikes`}>
      ...
      <AlertDialog open={!!deleteId} onOpenChange={() => setDeleteId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Remover bike</AlertDialogTitle>
            <AlertDialogDescription>
              {relatedCounts ? (
                <span>

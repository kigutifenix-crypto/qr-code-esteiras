...
import { ArrowLeft, Loader2, Save, AlertCircle, Bike } from 'lucide-react'

export default function NovaBikePage() {
  ...
  useEffect(() => {
    async function loadNumbers() {
      try {
        ...
        // Bikes usam prefixo B-
        const takenBikeNumbers = all
          .filter((t) => (t.equipmentType || 'esteira') === 'bike')
          .map((t) => t.qrCode)

        const allNumbers = Array.from({ length: 200 }, (_, i) => `B-${i + 1}`)
        const available = allNumbers.filter((n) => !takenBikeNumbers.includes(n))
        setAvailableNumbers(available)

        const bikeCount = takenBikeNumbers.length
        const nextNum = `B-${bikeCount + 1}`
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.number) {
      setError('Selecione um número para a bike.')
      setLoading(false)
      return
    }
    ...
    try {
      ...
      try {
        await createLog({
          ...
          details: `Created bike ${formData.name} (${formData.number})`,
        })
      } catch (e) {
      ...
      setError(err instanceof Error ? err.message : 'Erro ao criar bike')
    } finally {
      setLoading(false)
    }
  }
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-muted-foreground">
            Cadastre uma nova bike no sistema
          </p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <Card className="border-border/50">
          <CardHeader>
            ...
            <CardDescription>Dados de identificação da bike</CardDescription>
          </CardHeader>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  placeholder="Ex: Bike Spinning Pro"
                  required
        ...
        <Card className="border-border/50">
          <CardHeader>
            ...
            <CardDescription>Dados técnicos da bike</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="grid gap-4 sm:grid-cols-3">
              ...
              <div className="space-y-2">
                <Label htmlFor="motorPower">Potência / Resistência</Label>
                <Input
                  id="motorPower"
                  name="motorPower"
                  value={formData.motorPower}
                  onChange={handleChange}
                  placeholder="Ex: 8 níveis"
                  className="bg-input/50"
                />
              </div>
              <div className="space-y-2">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="space-y-2">
                <Label htmlFor="maxSpeed">Velocidade / Nível Máximo</Label>
                <Input
                  id="maxSpeed"
                  name="maxSpeed"
                  value={formData.maxSpeed}
                  onChange={handleChange}
                  placeholder="Ex: Nível 20"
                  className="bg-input/50"
                />
              </div>
              <div className="space-y-2">
                <Label htmlFor="incline">Ajuste de Selim</Label>
                <Input
                  id="incline"
                  name="incline"
                  value={formData.incline}
                  onChange={handleChange}
                  placeholder="Ex: 5 posições"
                  className="bg-input/50"
                />
              </div>
            </div>
        ...
        <div className="flex items-center gap-4">
          ...
          <Button type="button" variant="outline" asChild>
            <Link href="/dashboard/bikes">Cancelar</Link>
          </Button>
        </div>
      </form>
    </div>
